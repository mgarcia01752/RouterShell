...
class OsPrivilegeVerifier:
    ...
    @staticmethod
    def is_root_user() -> PredicateResult:
        """
        Check if the current user is the root user.
        
        Returns:
            StatusResult: True if the current user is root, False otherwise.
        """
        return os.geteuid() == 0

    @staticmethod
    def is_in_root_group() -> PredicateResult:
        """
        Check if the current user belongs to the root group.
        
        Returns:
            StatusResult: True if the current user is in the root group, False otherwise.
        """
        try:
    ...
    @staticmethod
    def has_root_privileges() -> PredicateResult:
        """
        Check if the user is root or in the root group.
        
        Returns:
            StatusResult: True if the user is root or in the root group, False otherwise.
        """
