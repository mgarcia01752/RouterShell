### Summary
Development installs now include Multipass VM tooling on apt/snapd systems while production installs remain unchanged. The installer installs snapd with the apt development package set, installs Multipass only for `--development`, and documents the VM workflow dependency.
...
- `command -v snap || true; apt-cache policy multipass ...; apt-cache policy snapd ...` → snap is installed, snapd is installed, no apt multipass candidate shown
- `/opt/routershell/venv/bin/python -m pytest tests/install/test_install_env.py -q` → pass, 8 passed
...
- `bash install/install.sh --help` → pass, help shows development VM test tooling
- `/opt/routershell/venv/bin/python -m pytest -q` → pass, 35 passed
- `/opt/routershell/venv/bin/python -m ruff check .` → pass
- `snap install multipass` → blocked, access denied without sudo
- `sudo -n snap install multipass` → blocked, sudo password required

### Tests
- `pytest` → pass, 35 passed
- `ruff` → pass, all checks passed
- Live Multipass install → blocked because this session cannot provide sudo password
