### Summary
Copied the PyPNM coding-agent rules into RouterShell as CODING_AGENT.md and added an AGENTS.md discovery pointer for future agents. Corrected copied license references to match RouterShell, added the private `RS` shorthand note, and added commit-message format rules based on PyPNM guidance.
...
- `sed -n '45,90p' /home/dev01/Projects/PyPNM/CODING_AGENTS.md` → pass; read PyPNM commit-message guidance.
- `sed -n '145,230p' CODING_AGENT.md` → pass; verified RouterShell commit-message guidance.
- `find . -name '__pycache__' -type d -print` → pass; no generated Python caches present.

### Tests
- `pytest` → not run; documentation-only update and pytest is not installed in this interpreter.
- `ruff` → not run; documentation-only update and Ruff is not installed in this interpreter.
- Markdown inspection → pass; updated section renders as normal Markdown bullets.

### Notes / Warnings
- `RS` is documented only as Maurice's shorthand for RouterShell, not as public project branding.
- `CODING_AGENT.md` still contains PyPNM-specific sections that should be adapted in a follow-up cleanup pass.

### Remaining TODOs / Follow-Ups
- Adapt remaining PyPNM-specific wording in `CODING_AGENT.md` to RouterShell-specific rules.
- Install development dependencies so pytest and Ruff can run instead of being skipped.
