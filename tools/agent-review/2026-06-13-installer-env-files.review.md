### Summary
The installer now creates a launcher-loaded RouterShell environment file, using a system env file for production installs and a repo-local .env for development installs by default. Install docs and tests cover the new --local-env, --global-env, and --global options plus launcher sourcing behavior.
...
- `/opt/routershell/venv/bin/python -m pytest tests/install/test_install_env.py` -> pass; 4 tests passed
...
- `/opt/routershell/venv/bin/ruff check tests/install/test_install_env.py --select I --diff` -> fail before import-order fix; showed Ruff's requested import layout

### Tests
- `pytest tests/install/test_install_env.py` -> pass; env option, scope selection, env creation, and launcher sourcing covered
- `pytest` -> pass; 18 tests passed through the QA checker
- `ruff` -> pass; All checks passed through the QA checker

### Notes / Warnings
- Existing env files are preserved and not overwritten by the installer.
- Production installs default to `/etc/routershell/routershell.env`; development installs default to repo-local `.env`.
- The repo-local `.env` path was already covered by `.gitignore`.
