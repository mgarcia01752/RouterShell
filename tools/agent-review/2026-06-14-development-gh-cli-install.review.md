### Summary
Added GitHub CLI installation to RouterShell development installs so failed GitHub Actions logs can be inspected from development workstations. Production installs remain unchanged, and development users can opt out with `ROUTERSHELL_INSTALL_GITHUB_CLI=false`.
...
- `/opt/routershell/venv/bin/python -m pytest tests/install/test_install_env.py -q` -> pass, 17 passed.
- `/opt/routershell/venv/bin/python -m ruff check tests/install/test_install_env.py` -> pass, all checks passed.
- `bash -n install/install.sh` -> pass.
- `/opt/routershell/venv/bin/python -m pytest -q` -> pass, 60 passed.
...
### Notes / Warnings
- `--development` installs `gh` through the detected package manager when it is missing.
- `ROUTERSHELL_INSTALL_GITHUB_CLI=false` skips the development GitHub CLI install.
- `--skip-os-packages` now fails development GitHub CLI installation when `gh` is missing.
...
def test_development_install_installs_github_cli_with_apt(tmp_path: Path) -> None:
    bin_dir = tmp_path / "bin"
    log_file = tmp_path / "apt.log"
    bin_dir.mkdir()
    apt_get = bin_dir / "apt-get"
    apt_get.write_text(f"#!/usr/bin/env bash\nprintf '%s\\n' \"$*\" >> {log_file}\n")
    apt_get.chmod(0o755)
...
"""
    ...
    assert log_file.read_text().strip() == "install -y gh"


def test_production_install_skips_development_github_cli(tmp_path: Path) -> None:
    bin_dir = tmp_path / "bin"
    log_file = tmp_path / "apt.log"
    bin_dir.mkdir()
    apt_get = bin_dir / "apt-get"
...
def test_development_github_cli_can_be_skipped_with_env(tmp_path: Path) -> None:
    bin_dir = tmp_path / "bin"
    log_file = tmp_path / "apt.log"
    bin_dir.mkdir()
    apt_get = bin_dir / "apt-get"
    ...
    script = f"""
export ROUTERSHELL_INSTALL_SH_NO_MAIN=true
export ROUTERSHELL_INSTALL_GITHUB_CLI=false
export PATH={bin_dir}:$PATH
source install/install.sh
DEVELOPMENT_INSTALL=true
PACKAGE_MANAGER=apt
...
def test_development_github_cli_requires_os_packages_when_missing() -> None:
    script = """
export ROUTERSHELL_INSTALL_SH_NO_MAIN=true
source install/install.sh
DEVELOPMENT_INSTALL=true
...
"""
    ...
    assert "Re-run without --skip-os-packages or install gh first" in result.stderr
