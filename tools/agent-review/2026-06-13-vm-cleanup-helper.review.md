### Summary
Added a RouterShell Multipass cleanup helper that deletes the configured VM by default and can optionally clean every `routershell-*` instance. The workflow docs and tests now cover dry-run, purge, and scoped cleanup behavior.

### Modified Files
- tools/vm/multipass-cleanup.sh
- tools/vm/README.md
- tests/tools/test_vm_workflow.py

### Commands Executed And Results
- `chmod +x tools/vm/multipass-cleanup.sh && bash -n tools/vm/multipass-cleanup.sh tools/vm/multipass-destroy.sh tools/vm/multipass-common.sh` → pass
- `/opt/routershell/venv/bin/python -m pytest tests/tools/test_vm_workflow.py -q` → pass, 5 passed
- `/opt/routershell/venv/bin/python -m ruff check tests/tools/test_vm_workflow.py` → pass
- `tools/vm/multipass-cleanup.sh --dry-run` → pass, would delete `routershell-install-test`
- `/opt/routershell/venv/bin/python -m pytest -q` → pass, 37 passed
...
- Cleanup dry run → pass, no VM deleted

# FILE: tools/vm/multipass-cleanup.sh
#!/usr/bin/env bash
set -euo pipefail
